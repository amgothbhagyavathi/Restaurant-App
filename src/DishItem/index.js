...
const DishItem = ({
  ...
  const onDecreamentQuantity = () => removeItemFromCart(dishDetails)
  ...
  const renderControllerButton = () => (
    <div className="controller-container d-flex align-items-center bg-success">
      <button className="button" type="button" onClick={onDecreamentQuantity}>
        -
      </button>
  ...
  return (
    <li className="mb-3 p-3 dish-item-container d-flex">
      ...
      >
        <div
          className={`veg-round ${dishType === 1 ? 'non-veg-border' : ''}`}
        />
      </div>
